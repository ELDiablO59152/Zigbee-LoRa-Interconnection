...
uint8_t voltmeterDec(void) { // renvoi la valeur d�cimale de l'adc sur 8 bits (ex : 82v)
...
void printBatteryLevel(void) { // affiche et renvoi le poucentage de batterie sur 10 bits (ex : 95.35%)
    ...
    uint16_t pourcentBatt = (uint16_t)(((float)vBatt / 10.0 - 700.0) / (840.0 - 700.0) * 10000);    // max batt 8,32 min batt 7 min r�g 5,3
