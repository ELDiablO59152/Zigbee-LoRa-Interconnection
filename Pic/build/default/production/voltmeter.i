...
uint16_t voltmeterHex(void) {
    ...
    value = ((uint16_t)ADRESH << 2) | ((uint16_t)ADRESL >> 6);
...
uint8_t voltmeterDec(void) {
    ...
    float volts = (float)((3.2 / 1023) * voltmeterHex());
...
void printBatteryLevel(void) {
    ...
    float volts = (float)((3.2 / 1023) * value);
    uint16_t integer = (uint16_t)(volts * 1000) % 10000;
    uint16_t vBatt = (uint16_t)(integer * 2.765);
    uint16_t pourcentBatt = (uint16_t)(((float)vBatt / 10.0 - 700.0) / (840.0 - 700.0) * 10000);
