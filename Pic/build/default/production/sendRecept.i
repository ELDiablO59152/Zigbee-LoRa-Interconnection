...
void Transmit(const uint8_t *data, const uint8_t data_long) {
    ...
    WriteSXRegister(0x22, data_long);

    for (i = 0; i < data_long; i++) {
        WriteSXRegister(0x00, data[i]);
    }
...
void Receive(uint8_t *data) {
    ...
    WriteSXRegister(0x0D, ReadSXRegister(0x0F));
    ...
    do {
        reg_val = ReadSXRegister(0x12);
    } while (((reg_val & 0x10) == 0x00) && ((reg_val & 0x80) == 0x00));

    if ((ReadSXRegister(0x12) & 0x10) == 0x00) {
...
# 142 "sendRecept.c"
    if((reg_val & 0x20) != 0x00){
        ...
        WriteSXRegister(0x0D, ReadSXRegister(0x10));
